...
class Ventas(tk.Frame):
    ...
    def __init__(self,padre):
        ...
        # Diccionario para llevar registro de modificaciones temporales al stock
        self.stock_temporal = {}
        self.widgets()
    ...
    def obtener_stock_actual(self, producto):
        """Obtiene el stock actual de un producto, considerando las modificaciones temporales"""
        try:
            conn = sqlite3.connect(self.db_name)
            c = conn.cursor()
            c.execute('SELECT stock FROM articulos WHERE articulo = ?', (producto,))
            stock_db = c.fetchone()[0]
            conn.close()
            
            # Resta cualquier cantidad que ya esté en el carrito
            if producto in self.stock_temporal:
                return stock_db - self.stock_temporal[producto]
            return stock_db
        except sqlite3.Error as e:
            print('Error obteniendo el stock actual: ', e)
            return 0

    def actualizar_visualizacion_stock(self):
        """Actualiza la visualización del stock en la interfaz"""
        producto_seleccionado = self.entry_producto.get()
        if producto_seleccionado:
            stock_actual = self.obtener_stock_actual(producto_seleccionado)
            self.label_stock.config(text=f'Stock: {stock_actual}')
    ...
    def agregar_articulo(self):
        ...
        try:
            ...
            precio, costo, stock_db = resultado
            ...
            if cantidad > stock_actual:
                messagebox.showerror('Error',f'No hay suficiente stock. Sólo hay {stock_actual} unidades disponibles.')
                return
            
            # Actualizar el stock temporal
            if producto in self.stock_temporal:
                self.stock_temporal[producto] += cantidad
            else:
                self.stock_temporal[producto] = cantidad
        ...
        self.actualizar_visualizacion_stock()
    ...
    def procesar_pago(self,cantidad_pagada,ventana_pago,total_venta):
        ...
        try:
            ...
            for item in self.productos_seleccionados:
                ...
                # No actualizamos el stock aquí porque ya lo hicimos en tiempo real
        ...
        # Limpiamos el registro de stocks temporales ya que la venta fue completada
        self.stock_temporal.clear()
        self.productos_seleccionados = []
        self.limpiar_campos()
    ...
    def limpiar_lista(self):
        # Restaurar el stock antes de limpiar
        self.restaurar_stock_temporal()
    ...
    def restaurar_stock_temporal(self):
        """Restaura el stock de los productos que estaban en el carrito"""
        self.stock_temporal.clear()
        self.actualizar_visualizacion_stock()
    ...
    def eliminar_articulo(self):
        ...
        # Restaurar el stock temporal para este producto
        if articulo in self.stock_temporal:
            self.stock_temporal[articulo] -= int(cantidad)
            if self.stock_temporal[articulo] <= 0:
                del self.stock_temporal[articulo]
        
        self.tre.delete(item_id)

        # Actualizar la lista de productos seleccionados
        self.productos_seleccionados = [producto for producto in self.productos_seleccionados 
                                    if not (producto[2] == articulo and producto[4] == int(cantidad))]
    ...
    def editar_articulo(self):
        ...
        if new_cantidad is not None:
            try:
                ...
                # Restaurar temporalmente la cantidad actual para calcular el stock disponible
                if current_producto in self.stock_temporal:
                    self.stock_temporal[current_producto] -= current_cantidad
                    if self.stock_temporal[current_producto] <= 0:
                        del self.stock_temporal[current_producto]
                
                # Verificar si hay suficiente stock para la nueva cantidad
                stock_actual = self.obtener_stock_actual(current_producto)
                
                if new_cantidad > stock_actual:
                    # Restaurar la cantidad original en el stock temporal
                    if current_producto in self.stock_temporal:
                        self.stock_temporal[current_producto] += current_cantidad
                    else:
                        self.stock_temporal[current_producto] = current_cantidad
                        
                    messagebox.showerror('Error',f'Stock insuficiente. Sólo hay {stock_actual} unidades disponibles.')
                    return
                
                # Actualizar el stock temporal con la nueva cantidad
                if current_producto in self.stock_temporal:
                    self.stock_temporal[current_producto] += new_cantidad
                else:
                    self.stock_temporal[current_producto] = new_cantidad
                ...
                for idx, producto in enumerate(self.productos_seleccionados):
                    if producto[2] == current_producto and producto[4] == current_cantidad:
...
#Edición de factura
    ...
    def widgets(self):
        ...
        boton_pagar = tk.Button(self, text='Pagar', font='sans 14 bold',comman=self.realizar_pago)
